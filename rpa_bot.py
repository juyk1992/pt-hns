        ...
        for port in ports_to_collect:
            ...
            for page_idx in range(1, total_pages + 1):
                ...
                for row_idx in range(10):
                    ...
                    # 💡 하역종료일 및 시간 파싱 (예: '2026-08-02 18:00' 형태) 및 KST 기준 24시간 이전 스킵 처리
                    ...
                    if len(end_clean_str) >= 12:
                        try:
                            ...
                            # 한국 시간(KST, UTC+9) 기준 현재 시각 및 24시간 이전 시점 산출
                            kst_now = datetime.utcnow() + timedelta(hours=9)
                            cutoff_dt = kst_now - timedelta(hours=24)
                            
                            if end_dt_obj < cutoff_dt:
                                print(f"⏭️ [{port_name} | {page_idx}p-{row_idx+1}행] {ship_name} ➔ 하역종료시각({haeyuk_end})이 한국 시간 기준 24시간 이전이므로 스킵")
                                # 목록 탭 복귀
                                driver.execute_script("""
