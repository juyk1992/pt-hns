...
def generate_gemini_summary(chem_name, unno, cas_no, dgst_info, safety_info, kosha_msds_text, rag_text, accident_context=""):
    ...
    try:
        ...
        수집된 아래 다중 데이터(공공 API, HNS DB, 해경 대응가이드 RAG)를 종합 분석하여, 현장 요원이 1초 만에 파악하고 지시할 수 있는 최적의 비상대응 가이드를 작성하세요.
        ...
        [작성 및 분석 엄격 규칙]
        1. [데이터 우선순위]: 이격거리, 대피거리, 필수 보호구 등 핵심 수치는 **해경 HNS 대응가이드(RAG) 및 HNS 정보집 DB 수치를 최우선 반영**하세요.
        2. [사고 상황 반영]: 만약 [현장 사고 상황 조건]이 존재할 경우, 단순 물질 특성을 넘어 **해당 사고 유형(좌초/유출/화재/침수 등)에 직결된 맞춤형 초동 조치**를 최우선 지침으로 작성하세요.
        3. [초동대응 핵심요약 표]: 서술형 문장을 절대 금지하며, **단어, 수치(M단위), 명사형 핵심 키워드 위주로 극도로 간결하게 작성**하세요. 정보가 없거나 불명확한 수치는 추정하지 말고 '정보없음(현장 확인 필요)'으로 명시하세요.
        4. [세부 지침 1~4번]: 현장 요원이 즉시 조치할 수 있도록 **모든 항목을 개조식(-, •)의 명확한 행동 지침 문장**으로 작성하세요.
        ...
        | **필수 보호구** | **[Level A/B/C/D]** + [공기호흡기/내화학복 등 필수장비] |
        | **초동 행동수칙** | **[풍상위치 확보]** + [사고상황 맞춤 행동 및 소화/방제 수칙 요약] |

        ---
        ### 1. ⚠️ 물리·화학적 성상 및 주요 위험성 (EmS/적재방법/주의사항)
        ### 2. 🛡️ 현장 개인 보호구 및 초동 방제/소화 요령 (사고 상황 맞춤 대응)
