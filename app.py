...
def generate_gemini_vision_summary(
  ...
  try:
    ...
    prompt_text = f"""
        ...
        첨부된 [HNS 정보집 원본 스캔 이미지]와 [해양사고 대응 가이드 원본 스캔 이미지들], 그리고 수집된 다중 데이터(공공 API, MSDS, RAG 텍스트)를 철저히 교차 분석하여, 현장 세력(OSC, 경비함정, 방제함정, 특수구조대 등)에 즉각 하달할 수 있는 가장 전문적이고 완벽한 **비상대응 지시서**를 가독성있게 작성하세요.
        ...
        4. **출력 형식 엄격 준수**: 공문서 서식(수신, 발신 등)을 절대 생성하지 말고, 반드시 `### 🚨 [초동대응 핵심요약]` 제목부터 곧바로 출력을 시작하세요. [초동대응 핵심요약]은 나열식이므로 가독성있게 줄바꿈하세요.
